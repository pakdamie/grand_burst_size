...
FULL_MODEL_SIMULATING_Duration_criss_cross <- function(value_interest, 
  ...
  RM_limit_1 <- Calculate_Initial_RM_CrissCross(species, value_interest)
  ...
  Fitness_MODEL_FULL$variable_interest <- value_interest
  
  ### Write into a CSV TO BE SAVED 
  write.csv(Fitness_MODEL_FULL, file = here(
    "Output", "Fitness_Model",
    paste("FITNESS_MODEL_",species,value_interest,".csv", sep = "")))
