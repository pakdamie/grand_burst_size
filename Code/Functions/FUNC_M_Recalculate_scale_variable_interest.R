#' Rescale the multiplier to the original scale
#'
#' @param PC_fit_dat 
#' @param PF_fit_dat 
#' @param variable_interest alpha1, R_modifier, combo
#'
#' @return data.frame
#' @export
#'
#' @examples
Rescale_Rate_Variable_Interest <- function(
  PC_fit_dat,PF_fit_dat,
  variable_interest)
  {
  ...
  if (variable_interest == 'combo'){
     ...
     levels(Fitness_PF$mu_M_rescale) <-  
        c(levels(Fitness_PF$mu_M_rescale),"1")
  ...
  if (variable_interest != "combo"){
  Fitness_PC$controlornot <- ifelse(
    Fitness_PC[,variable_interest] == 1, 'control','not')
  
  
  Fitness_PF$controlornot <- ifelse(
    Fitness_PF[,variable_interest] == 1, 'control','not')
  }
  else{
  Fitness_PC$controlornot <- ifelse(
    Fitness_PC[,c("mu_M", "p_val")] == 1, 'control','not')

  Fitness_PF$controlornot <- ifelse(
    Fitness_PF[,c("mu_M", "p_val")] == 1, 'control','not')
