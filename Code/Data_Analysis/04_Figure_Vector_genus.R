...
mal_dat_asex_vector_sep_melted <- na.omit(
  melt(mal_dat_asex_vector_sep, 
...
mal_dat_asex_known_vector$value <- factor(
  ...
  levels = c("Anopheles", "Aedes", 
             "Coquillettidia", "Culex", 
             "Culiseta", "Lutzomyia")
)

# Plot the data
ggplot(mal_dat_asex_known_vector, 
  aes(x = value, y = Upper)) +
...
# Save the plot
ggsave(here("Figures", "Raw", "Vector_genus.pdf"), 
