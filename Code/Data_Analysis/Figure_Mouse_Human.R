...
Muridae_DF <- subset(mal_dat_asex_full,
       mal_dat_asex_full$Plasmodium.species %in% c("aegyptensis","berghei", "chabaudi",
                                                   "vinckei"))

Muridae_DF$highlight <- "No"

Muridae_DF[Muridae_DF$Plasmodium.species %in% c("berghei", "chabaudi"),]$highlight = "Yes" 


Mouse_Burst_GG <- ggplot(Muridae_DF, aes(x = Upper, y = Plasmodium.species, color = highlight ))+ 
        geom_point(size=3)+
...
Human_Burst_GG <- ggplot(Human_DF , aes(x = Upper, y = Plasmodium.species, color = highlight ))+ 
        geom_point(size=3)+
        geom_segment(aes(x = Lower, xend = Upper, y = Plasmodium.species,
                         yend = Plasmodium.species),size =0.8)+
        ylab("")+
