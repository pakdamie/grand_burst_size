...
mal_dat_asex_vector_sep <- separate_rows(
  mal_dat_asex_vector, "Vector_Genus",
  "Experimental_Suspected_Vector_Genus"
)
...
mal_dat_asex_vector_sep_melted <- na.omit(melt(mal_dat_asex_vector_sep, 
...
ggplot(mal_dat_asex_known_vector, aes(x = value, y = Upper)) +
  ...
  scale_color_manual(values = c('avian' = "#30d5c8", 'mammal' = "#fe64a3",
                                'reptile' = "#424b54"
                                ))+
  xlab("Vector genus") +
