...
load(here("Output", "brms_output", "fit_simp.R"))
...
each Nexus file contains a distribution of possible trees ($n$ = 1000). We construct a main consensus tree for each host clade using the 50% majority rule and randomly resolving polytomies. Note that there are no branch lengths when creating the consensus tree.
...
For the \textit{Plasmodium} phylogeny, we use the single phylogeny tree provided by Galen et al. 2018 [@galen2018]:
...
Example: For Plasmodium floridense, the species can have varying upper burst sizes depending on the host!
```{r}

print(mal_dat_asex[20:30,c("Plasmodium_species","Host", "Upper_Burst_Size")])
```

To test for the phylogenetic signal, we construct a random intercept
model that only includes the grand intercept and the random effects. Here,
we describe the observed maximum burst size $i$ belonging to the
\textit{Plasmodium} species $j$ in host $k$: $(Y_{j{i}k{i}})$.

We have five random effects: the parasite ($\alpha$) and host ($\phi$)
membership accounting for phylogeny, parasite ($\gamma$) and host
($\delta$) species name accounting for the 'ecology' of the species, and
the individual observations ($\eta$). By including the individual
observations as a random-effect, we can account for the dispersion in
the data [@harrison2014]. The full model formulations are below:
...
$\Sigma_\alpha$ and $\Sigma_\phi$ describing the variance-covariance matrices calculated from phylogenetic trees (describes the correlation structure
between observations $i$). For $\gamma_{j[i]}$ and $\delta_{k[i]}$,
these are the random effects due to the species name with variance
$\sigma_{\gamma}^2$ and $\sigma_{\delta}^2$, respectively. The
individual-level random effect is then explained by the variance
$\sigma_\eta^2$.

The simpler alternative model only investigates the roles of the
host and parasite phylogenies:

$$
\begin{aligned}
  \operatorname{Y}_{i}  &\sim \operatorname{Poisson}(\lambda_i) \\
    \log(\lambda_i) &= \beta_0 + \alpha_{j[i]} + \phi_{k[i]} + \eta_i \\
    \alpha_{j[i]}  &\sim N \left(0, \sigma_{\alpha}^2\Sigma_{\alpha} \right)
    \text{, for Plasmodium species (phy) j = 1,} \dots \text{, 24} \\
    \phi_{k[i]}  &\sim N \left(0, \sigma_{\phi}^2\Sigma_{\phi} \right)
    \text{, for Host species (phy) k = 1,} \dots \text{,52}\\
...
For the grand intercept $B_0$, this is the grand mean across all
observations.

```{r}
ggplot(mal_dat_asex, aes(x= Upper_Burst_Size)) + geom_histogram(bins = 40, fill = 'black') + 
  xlab("Maximum observed burst size")+
  ylab("Counts")+
theme_classic()+
...
Calculating the mean of the upper burst size:

```{r}
log(mean(mal_dat_asex$Upper_Burst_Size,na.rm = TRUE))
###The mean (log) is 3.06 - for the prior 
```

We assume a student-t test with a degree of freedom of 3, mean ($\mu$)
of 3, and the scale parameter of 1.0 Note, that this is similar to the
default prior chosen by brms for the intercept. Because the maximum
observed burst size is a positive integer, we set the lower bound as 0.

#### The parasite's ecology and evolutionary history

Our prior expectation is that the parasite ecology and evolutionary
history have a stronger role in influencing the maximum observed burst
size. Specifically, we observe that for Plasmodium species that exploit
multiple hosts, the maximum observed burst sizes do not vary as significantly. Therefore, the parasite random effects should have a larger value than the host random effects. For all priors, we assume a student-t test (similar to a normal distribution with fat tails). In the full complete model, convergence is difficult and requires very sharp priors.

## Running the full model in brms

### Calculating the covariance matrices

We use the compute.brlen to calculate the branch length and the vcv to construct the variance covariance matrices.
...
Having more species on the phylogenetic tree increase the overall branch
lengths. For comparison, we standardize the variance covariance matrices
by dividing each matrix by its determinant:
...
model_null_ecophylo<- brm(
  Upper_Burst_Size ~ 1 +
    ...
    (1 | obs),
  prior = c(
    prior(student_t(4, 0, 1), "sd",lb = 0, ub = 3, group = "obs"),
    prior(student_t(4, 0, 1), "sd", lb = 0, ub = 3, group = "Host_name"
    ),
     prior(student_t(4, 0, 1), "sd", lb = 0, ub = 3, group = "Host"),
    prior(student_t(4, 0, 1), "sd", lb = 0, ub = 3, group = "Parasite_name"),
    prior(student_t(4, 0, 1), "sd", lb = 0, ub = 3, group = "Plasmodium_species"),
      prior(student_t(3, 3, 1.0), "Intercept", lb = 0)
  ),
  data = mal_dat_asex,
  family = poisson("log"),
  warmup = 4000,
...
```{r message=FALSE, warning=FALSE, eval = FALSE, echo = TRUE}
model_null_ecophylo_simpler <- brm(
  Upper_Burst_Size ~ 1 +
    (1 | gr(Plasmodium_species, cov = vcv_Plasmodium_standard)) +
    (1 | gr(Host, cov = vcv_TypeHost_standard)) +
    (1 | obs),
  prior = c(
    prior(student_t(4, 0, 1), "sd",lb = 0, ub = 3,group = "obs"),
     prior(student_t(4, 0, 1), "sd", lb = 0, ub = 3, group = "Host"),
    prior(student_t(4, 0, 1), "sd", lb = 0, ub = 3, group = "Plasmodium_species"),
      prior(student_t(3, 0, 1.0), "Intercept", lb = 0)
  ),
  data = mal_dat_asex,
  family = poisson("log"),
  warmup = 4000,
...
## Model validation
We then do a 'loo' model validation (leave-one-out):

```{r, echo = TRUE, eval = FALSE}
fit_orig <- add_criterion(model_null_ecophylo , "loo", reloo =
TRUE)
fit_simp <- add_criterion(model_null_ecophylo_simpler,
"loo", reloo = TRUE)
```

```{r}
loo_compare(fit_orig,fit_simp,criterion = "loo")
```

The difference in the model prediction is very slight- we opt for the simpler model that has better convergence.

Additionally, we then check if the two models describe the observed data well by simulating from the posterior predictive distribution (pp_check). The black line is the observed maximum observed burst size while the faint blue lines describe the random draws from the posterior distribution. For both models, they seem to fit well.

```{r}
pp_check(model_null_ecophylo,type= "intervals",ndraws= 10)
###pp_check(fit_orig, ndraws = 1000)
check_predictions(fit_orig, type = "discrete_both")

pp_check(fit_simp,ndraws = 1000)
```
...
methods: (1) comparing the loo of models only containing the host and parasite phylogenies, (2) and by calculating the variance attributed to either host and parasite phylogeny over the total variance.
...
```{r, echo = TRUE, eval = FALSE}
model_host_only_ecophylo <- brm( Upper_Burst_Size ~ 1 +
(1|gr(Host , cov = vcv_TypeHost_standard))+ (1|obs),
...
```{r}
loo_compare(fit_simp,
            fit_par_only,
            fit_host_only,
            criterion = "loo")
```

The original model (fit_simp) containing both the host and parasite phylogenies is the most similar to the model containing only the parasite phylogeny (fit_par_only). The fit_host_only is significantly different from the first two models.

#### Host and Parasite Effects

We borrow code from the {brms} forum, where we draw from the posterior distribution to obtain the standard deviations of the random effects. These are then squared to get the variances. The total variance is then quantified by summing all the variances. Each random effects are then divided by the total variance to get the proportion of the variance explained.

```{r}
variance_partition <-fit_simp %>% 
  tidy_draws() %>% 
...
```{r}
ggplot(variance_partition[c(5,7),], aes(x = component, y = y))+
  geom_point(size =4) + 
...
This plot shows the credible intervals for the proportion of variance explained by either the host or parasite phylogeny. One can see that parasite phylogeny is the most important driver 